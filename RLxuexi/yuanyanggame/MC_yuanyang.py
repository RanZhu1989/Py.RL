...
class YuanYangEnv:
    ...
    #这是一个用于蒙特卡洛方法的环境，增加了q显示（格子四个方向）
    #注意！用蒙特卡洛方法时需要将稀疏奖励稠密化
    ...
    def __init__(self):
        ...
        self.gamma=0.95                         #这里gamma变大了
        self.value=np.zeros((10,10))            #初始化v(s)
        self.Qtable=np.zeros((100,4))           #初始化q表,列 1=up 2=down 3=left 4=right
    ...
    def render(self):
        ...
        for state in range(100):
            ...
            surface=self.font.render(str(round(float(self.Qtable[state,0]),2)),True,(0,0,0))#打印往上的q
            self.viewer.blit(surface,((j*120+50),(i*90+10)))
            surface=self.font.render(str(round(float(self.Qtable[state,1]),2)),True,(0,0,0))#打印往下的q
            self.viewer.blit(surface,((j*120+50),(i*90+70)))
            surface=self.font.render(str(round(float(self.Qtable[state,2]),2)),True,(0,0,0))#打印往左的q
            self.viewer.blit(surface,((j*120+10),(i*90+45)))
            surface=self.font.render(str(round(float(self.Qtable[state,3]),2)),True,(0,0,0))#打印往右的q
            self.viewer.blit(surface,((j*120+80),(i*90+45)))
        #画路径点
